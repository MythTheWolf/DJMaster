...
public class DJMasterMain implements BotPlugin {
	...
	public boolean onEnable(DiscordPlugin arg0) {
		
		File config = arg0.getConfig();
		InputStream internal = arg0.getInternalResource(arg0.getSelfJar(), "config.json");
		if (!config.exists()) {
			try {
		...
		try {
			...
			arg0.registerCommand("!stop", exec -> {
				...
				List<StreamPlayer> PL = Maps.PLAYERS.stream()
						.filter(it -> it.getDJ() != null && it.getDJ().getId().equals(ID)).collect(Collectors.toList());
				if (!(PL.size() > 0)) {
